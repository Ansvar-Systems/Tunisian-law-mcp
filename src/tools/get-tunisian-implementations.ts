/**
 * get_tunisian_implementations — Find Tunisian statutes implementing a specific EU directive/regulation
...
export interface GetTunisianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface TunisianImplementationResult {
  document_id: string;
...
export async function getTunisianImplementations(
  db: InstanceType<typeof Database>,
  input: GetTunisianImplementationsInput,
): Promise<ToolResponse<TunisianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as TunisianImplementationResult[];
