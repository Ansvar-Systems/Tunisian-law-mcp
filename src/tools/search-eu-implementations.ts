/**
 * search_eu_implementations — Search EU directives/regulations that have Tunisian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_tunisian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as tunisian_statute_count
  ...
  if (input.has_tunisian_implementation) {
    sql += ' HAVING tunisian_statute_count > 0';
  }
