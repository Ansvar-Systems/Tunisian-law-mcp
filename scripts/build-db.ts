#!/usr/bin/env tsx
/**
 * Database builder for Tunisia Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Tunisia Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Tunisian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'TN');
    insertMeta.run('source', 'legislation.tn');
