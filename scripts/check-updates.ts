#!/usr/bin/env tsx
/**
 * Tunisian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://legislation.tn';
const PORTAL_NAME = 'Tunisia Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/tunisian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Tunisian Law MCP — Data Freshness Check');
