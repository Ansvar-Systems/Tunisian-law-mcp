#!/usr/bin/env tsx
/**
 * Drift detection for Tunisia Law MCP.
 *
 * Checks if upstream Tunisia Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Tunisia Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'tunisian-law-mcp/1.0 drift-detect' },
      });
